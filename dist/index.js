...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            case 'get_issues': {
                const issues = await sonarClient.getIssues(args?.projectKey, args?.types, args?.severities, args?.statuses, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Found ${issues.length} issues${args?.branch ? ` (Branch: ${args.branch})` : ''}:\n\n${issues.map(issue => `**${issue.severity}** - ${issue.type}\n` +
                                `Rule: ${issue.rule}\n` +
            ...
            case 'get_code_smells': {
                const codeSmells = await sonarClient.getCodeSmells(args?.projectKey, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Found ${codeSmells.length} code smells:\n\n${codeSmells.map(issue => `**${issue.severity}** - ${issue.component}:${issue.line || 'N/A'}\n` +
                                `Rule: ${issue.rule}\n` +
                                `Message: ${issue.message}\n` +
                                `Status: ${issue.status}\n`).join('\n---\n')}`,
                        },
            ...
            case 'get_bugs': {
                const bugs = await sonarClient.getBugs(args?.projectKey, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Found ${bugs.length} bugs:\n\n${bugs.map(issue => `**${issue.severity}** - ${issue.component}:${issue.line || 'N/A'}\n` +
                                `Rule: ${issue.rule}\n` +
                                `Message: ${issue.message}\n` +
                                `Status: ${issue.status}\n`).join('\n---\n')}`,
                        },
            ...
            case 'get_vulnerabilities': {
                const vulnerabilities = await sonarClient.getVulnerabilities(args?.projectKey, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Found ${vulnerabilities.length} vulnerabilities:\n\n${vulnerabilities.map(issue => `**${issue.severity}** - ${issue.component}:${issue.line || 'N/A'}\n` +
                                `Rule: ${issue.rule}\n` +
                                `Message: ${issue.message}\n` +
                                `Status: ${issue.status}\n`).join('\n---\n')}`,
                        },
            ...
            case 'get_security_hotspots': {
                const hotspots = await sonarClient.getSecurityHotspots(args?.projectKey, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Found ${hotspots.length} security hotspots:\n\n${hotspots.map(issue => `**${issue.severity}** - ${issue.component}:${issue.line || 'N/A'}\n` +
                                `Rule: ${issue.rule}\n` +
                                `Message: ${issue.message}\n` +
                                `Status: ${issue.status}\n`).join('\n---\n')}`,
                        },
            ...
            case 'get_coverage': {
                const coverage = await sonarClient.getCoverage(args?.projectKey, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
            ...
            case 'get_metrics': {
                const metrics = await sonarClient.getMetrics(args?.projectKey, args?.metricKeys, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Metrics for ${metrics.component.key}:\n\n${metrics.component.measures?.map(metric => `**${metric.metric}**: ${metric.value}${metric.periods ? ` (Period: ${metric.periods[0]?.value})` : ''}`).join('\n') || 'No metrics available'}`,
                        },
                    ],
                };
            }
            case 'get_project_status': {
                const status = await sonarClient.getProjectStatus(args?.projectKey, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Project Status:\n\n` +
                                ...
                                `Conditions:\n${status.projectStatus.conditions?.map((condition) => `- ${condition.metricKey}: ${condition.actualValue} (${condition.status})`).join('\n') || 'No conditions'}`,
                        },
                    ],
                };
            }
            case 'get_components': {
                const components = await sonarClient.getComponents(args?.projectKey, args?.branch, args?.pullRequest);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Project Components (${components.components.length} files):\n\n${components.components.map((comp) => `**${comp.name}** (${comp.qualifier})\n` +
                                `Key: ${comp.key}\n` +
                                `Path: ${comp.path}\n`).join('\n---\n')}`,
                        },
                    ],
            ...
            case 'get_branches': {
                const branches = await sonarClient.getBranches(args?.projectKey);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Available Branches:\n\n${branches.branches?.map((branch) => `**${branch.name}** (${branch.type})\n` +
            ...
            case 'get_pull_requests': {
                const pullRequests = await sonarClient.getPullRequests(args?.projectKey);
                return {
                    content: [
                        {
                            type: 'text',
                            text: `Available Pull Requests:\n\n${pullRequests.pullRequests?.map((pr) => `**PR #${pr.key}** - ${pr.title}\n` +
                                `Branch: ${pr.branch}\n` +
                                `Base: ${pr.base}\n` +
        ...
        throw new McpError(ErrorCode.InternalError, `Error calling SonarQube API: ${error instanceof Error ? error.message : String(error)}`);
    }
});
async function main() {
